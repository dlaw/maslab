...
def select(np.ndarray[DTYPE_t, ndim=3] hsv not None,
           np.ndarray[np.uint8_t, ndim=2] targets not None,
           np.ndarray[np.uint16_t, ndim=2] scalers not None):
    """
    ...
    This function returns an array where each pixel is the index
    of the highest priority color matched, or 0xFF if none were matched.
    """
    ...
    cdef int num_colors = targets.shape[0], i, j, c
